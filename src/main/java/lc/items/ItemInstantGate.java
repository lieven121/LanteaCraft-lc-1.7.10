...
@Definition(name = "Instant Gate", type = ComponentType.STARGATE, itemClass = ItemInstantGate.class)
public class ItemInstantGate extends LCItem {

	/** Display icon */
	public IIcon icon;

	public ItemInstantGate() {
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int p_77648_7_,
			...
			StructureStargate.generateStructureCenter(world, x, y, z, direction);
			
			
			//LCLog.debug(MathHelper.floor_double((double)(player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3);
	...
	public void registerIcons(IIconRegister ir) {
		icon = ir.registerIcon(ResourceAccess.formatResourceName("${ASSET_KEY}:creative_icon"));
	}
