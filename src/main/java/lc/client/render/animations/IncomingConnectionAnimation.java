...
public class IncomingConnectionAnimation extends Animation {

	public IncomingConnectionAnimation(Double duration, Double start, Double end, boolean resample) {
		super(duration, resample, new RunnableTileCallback() {
			...
			public void run(LCTile tile) {
				if ((StargateType.fromOrdinal(tile.getBlockMetadata()) == StargateType.STANDARD)) {
					...
					tile.mixer().replayChannel("spin_alt");
			}
		...
		addProperty("incomming-rotation", start, end, InterpolationMode.SMOOTHSTEP);
